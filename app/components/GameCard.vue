...
// Format start time for upcoming games
...
const getStatusDisplay = computed(() => {
  if (isUpcoming(props.game.status.state)) {
    // For upcoming games, show the start time
  ...
  >
    ...
    <div class="px-6 py-4">
      ...
      <div class="flex items-center justify-between mb-4 group/team">
        <div class="flex items-center gap-3 flex-1">
          <div class="relative">
            ...
            />
            <div v-if="game.awayTeam.winner && !isUpcoming(game.status.state)" class="absolute -top-1 -right-1 w-4 h-4 bg-yellow-400 rounded-full flex items-center justify-center">
              <span class="text-[8px]">👑</span>
            </div>
          </div>
          <div class="flex-1 min-w-0">
            <p 
              class="font-orbitron font-bold text-white truncate transition-colors"
        ...
        <div v-if="!isUpcoming(game.status.state)"
          class="font-orbitron text-3xl font-black tabular-nums"
      ...
      <div class="relative h-px bg-gradient-to-r from-transparent via-ice-500/30 to-transparent my-3">
        <span v-if="isUpcoming(game.status.state)" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-puck px-3 py-1 text-sm text-yellow-400 font-orbitron font-bold">VS</span>
        <span v-else class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-puck px-2 text-xs text-gray-500 font-mono">vs</span>
      </div>
        ...
        <div class="flex items-center gap-3 flex-1">
          <div class="relative">
            <img 
              ...
              class="w-12 h-12 object-contain transition-transform duration-300 group-hover/team:scale-110"
            />
            <div v-if="game.homeTeam.winner && !isUpcoming(game.status.state)" class="absolute -top-1 -right-1 w-4 h-4 bg-yellow-400 rounded-full flex items-center justify-center">
              <span class="text-[8px]">👑</span>
            </div>
          </div>
          <div class="flex-1 min-w-0">
    ...
    <!-- Odds Section -->
    <div v-if="game.odds && isUpcoming(game.status.state)" class="px-6 py-3 bg-green-500/5 border-t border-green-500/20">
    ...
    <!-- Hover glow effect -->
    <div class="absolute inset-0 bg-gradient-to-br from-ice-500/0 via-ice-500/0 to-ice-500/0 group-hover:from-ice-500/5 group-hover:via-ice-400/5 group-hover:to-ice-300/5 transition-all duration-500 pointer-events-none" />
  </NuxtLink>
</template>
