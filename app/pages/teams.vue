...
const getFavoriteTeamData = (team: typeof teams.value[0]) => ({
  id: Number(team.id),
...
<template>
  <div class="min-h-screen p-4 md:p-8">
    ...
    <div class="max-w-md mx-auto mb-8">
      <div class="relative">
        <input v-model="searchQuery" type="text" placeholder="Search teams..."
    ...
    <div v-else class="max-w-7xl mx-auto">
      ...
      <div v-for="(groupTeams, letter) in groupedTeams" :key="letter" class="mb-10">
        <div class="flex items-center gap-3 mb-4">
          <span
            class="w-10 h-10 bg-ice-600/20 rounded-xl flex items-center justify-center font-orbitron font-bold text-ice-400 text-xl">
            {{ letter }}
          </span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          <div v-for="team in groupTeams" :key="team.id"
            ...
            <div class="p-5">
              <div class="flex items-center gap-4">
                ...
                <div
                  ...
                  <img :src="team.logo" :alt="team.displayName" class="w-full h-full object-contain" />
                </div>

                <!-- Info -->
                <div class="flex-1 min-w-0">
                  <h3
                    class="font-orbitron font-bold text-white text-lg truncate group-hover:text-ice-300 transition-colors">
                    {{ team.location }}
                  </h3>
                ...
                <ClientOnly>
                  <FavoriteButton type="team" :team="getFavoriteTeamData(team)" />
                </ClientOnly>
              </div>

              <!-- Links -->
              <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-ice-900/20">
                <a v-if="team.links.clubhouse" :href="team.links.clubhouse" target="_blank" rel="noopener noreferrer"
                ...
                <a v-if="team.links.roster" :href="team.links.roster" target="_blank" rel="noopener noreferrer"
                  class="px-3 py-1.5 text-xs font-mono bg-ice-600/10 hover:bg-ice-600/20 text-ice-400 rounded-lg transition-colors flex items-center gap-1">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a v-if="team.links.stats" :href="team.links.stats" target="_blank" rel="noopener noreferrer"
                  class="px-3 py-1.5 text-xs font-mono bg-ice-600/10 hover:bg-ice-600/20 text-ice-400 rounded-lg transition-colors flex items-center gap-1">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <NuxtLink :to="`/team/${team.id}/schedule`"
