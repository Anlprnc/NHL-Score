...
<template>
  <div class="min-h-screen p-4 md:p-8">
    ...
    <div v-else-if="game" class="max-w-5xl mx-auto">
      ...
      >
        <!-- Live indicator -->
        <div v-if="isLive" class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-red-500 via-orange-500 to-red-500 animate-pulse" />

        <div class="p-6 md:p-10">
          <!-- Status badge -->
          <div class="flex justify-center mb-6">
            <span 
              :class="[getStatusBg(game.status.state), getStatusColor(game.status.state)]"
          ...
          <div class="flex items-center justify-center gap-8 md:gap-16">
            ...
            <div class="flex flex-col items-center text-center">
              ...
              />
              <h2 
                class="font-orbitron font-bold text-xl md:text-2xl"
              ...
              <div 
                class="font-orbitron text-5xl md:text-7xl font-black mt-4 tabular-nums"
            ...
            <table class="w-full min-w-[300px]">
              <thead>
                <tr class="text-gray-500 font-mono text-xs uppercase tracking-wider">
                  ...
                  <th v-for="period in game.periodScores" :key="period.period" class="text-center py-2 px-3 min-w-[40px]">
                    {{ period.displayValue }}
                  </th>
              ...
              <tbody>
                <tr class="text-white">
                  ...
                  <td v-for="period in game.periodScores" :key="`away-${period.period}`" class="text-center py-2 px-3 font-mono">
                    {{ period.awayScore }}
                  </td>
                  <td class="text-center py-2 px-3 font-orbitron font-bold border-l border-ice-900/30" :class="game.awayTeam.winner ? 'text-ice-300' : ''">
                    {{ game.awayTeam.score }}
                  </td>
                </tr>
                <tr class="text-white">
                  ...
                  <td v-for="period in game.periodScores" :key="`home-${period.period}`" class="text-center py-2 px-3 font-mono">
                    {{ period.homeScore }}
                  </td>
                  <td class="text-center py-2 px-3 font-orbitron font-bold border-l border-ice-900/30" :class="game.homeTeam.winner ? 'text-ice-300' : ''">
                    {{ game.homeTeam.score }}
                  </td>
                </tr>
              </tbody>
      ...
      <!-- Scoring Summary -->
      <div v-if="Object.keys(scoringByPeriod).length" class="bg-gradient-to-br from-puck-light/80 to-puck/80 backdrop-blur-sm rounded-2xl border border-ice-900/30 overflow-hidden mb-8">
      ...
      <!-- No Scoring -->
      <div v-else-if="game.status.state !== 'pre'" class="bg-gradient-to-br from-puck-light/80 to-puck/80 backdrop-blur-sm rounded-2xl border border-ice-900/30 p-8 text-center mb-8">
        <span class="text-4xl mb-4 block">🥅</span>
        <p class="text-gray-400 font-mono">No goals scored yet</p>
      </div>

      <!-- Team Stats -->
      <div v-if="game.teamStats.home.length || game.teamStats.away.length" class="bg-gradient-to-br from-puck-light/80 to-puck/80 backdrop-blur-sm rounded-2xl border border-ice-900/30 overflow-hidden">
        <div class="p-6 border-b border-ice-900/30">
          <h3 class="font-orbitron text-xl text-ice-400 font-bold flex items-center gap-3">
            <span class="text-2xl">📊</span>
            Team Statistics
        ...
        <div class="p-4 md:p-6">
          ...
          <div class="space-y-3">
            <div 
              v-for="(stat, index) in game.teamStats.home" 
              :key="stat.name"
